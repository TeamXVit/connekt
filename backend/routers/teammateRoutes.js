...
teammateRouter.post("/post",authenticateToken, async (request,response)=>{
    try{
        ...
        return response.status(200).send({
            message:"TeamMate Post created successfully."
        });
    }catch(err){
...
teammateRouter.get("/view", authenticateToken, async (request, response) => {
    try {
        ...
        const changeStream = TeamMate.watch();
        async function sendPosts(){
            try {
                const posts = await TeamMate.find()
...
teammateRouter.post("/comment/:id",authenticateToken, async (request, response)=>{
    try{
        const {regno} = request.user;
        ...
        const post = await TeamMate.findById(id);
...
teammateRouter.delete("/comment/:id", authenticateToken, async (request, response)=>{
    try{
        const { regno } = request.user;
        const { id } = request.params;
        const user = await Users.findOne({regno});
        ...
        const post = await TeamMate.findOne({"comments._id":id});
        if(!post) return response.status(404).send({
            error: "Post not found."
        });
...
teammateRouter.delete("/delete/:id", authenticateToken, async (request, response)=>{
    try{
        const {regno} = request.user;
        const {id} = request.params;
        const user = await Users.findOne({regno});
